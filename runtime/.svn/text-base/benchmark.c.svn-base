...
#define BENCHMARK_ITERATION 100000000
#define PIC_DEGREE 1
...
void printIntegerResult(id anObject)
{
	...
	int i;
	SEL selector = register_typed_selector("getInteger", "i@:");
	SLOT_LOOKUP(anObject, selector, nil,
			{
...
int main(void)
{
	...
	SEL new_selector = register_typed_selector("new", "@@:");
	SLOT_LOOKUP(AnObject, new_selector, nil, {
			anObject = MSG_SEND(AnObject);
			});
	clock_t c1, c2;
	
	SEL selector = register_typed_selector("addToInteger:", "v@:i");
	/* Normal Benchmark */
	c1 = clock();
	...
	{
		//[anObject addToInteger:1];
		AnObject_addToInteger((void*)anObject, NULL, 1);
	}
	c2 = clock();
	printf("Direct (C) call took %f seconds.\n", ((double)c2 - (double)c1)/ (double)CLOCKS_PER_SEC);
#if 0
	/* MIC Benchmark */
	c1 = clock();
	for(unsigned long long i=0 ; i<BENCHMARK_ITERATION ; i++)
	...
	c2 = clock();
	printf("MIC version took %f seconds.\n", ((double)c2 - (double)c1)/ (double)CLOCKS_PER_SEC);
	printIntegerResult(anObject);
	...
	c2 = clock();
	printf("PIC version took %f seconds.\n", ((double)c2 - (double)c1)/ (double)CLOCKS_PER_SEC);
	printIntegerResult(anObject);
#endif
	return 0;
}
